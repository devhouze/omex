...
(function($) {
    ...
    function _isFocused($elem) {
        if ($elem.is(":focus")) { return true; } else { return false; }
    }

    // check if some $elem's child is focused
    function _isChildFocused($elem) {
        if ($elem.find(":focus").length) { return true; } else { return false; }
    }
    ...
    function _isNormalTime(time_str) {
        ...
        if (match1) {
            ...
            // if something else is included, this is invalid
            if (match2[0] !== time_str) { return false; } else { return true; }
        }
    }
    ...
    function _timeToInt(time_str) {
        ...
        } else if (time_str.search("noon") != -1) { return 12 * 60; } else if (time_str.search("midnight") != -1) { return 0; } else { return null; }
    }
    ...
    function _formattedTimeToInt(time_str, format, ampm_text) {
        ...
        if (!placeholders.minute) { return null; } else if (placeholders.ampm && placeholders.hour_ampm) {
    ...
    function _getTimeDuration(start, end) {
        if (start > end) { return end + 24 * 60 - start; } else { return end - start; }
    }
    ...
    function _bindTimepicker($picker, $input) {
        ...
        if (settings.use12HourClock) {
            $picker.find(".ampm-button").on("click", function() {
                var $this = $(this);
                if ($this.html() === "pm") { $this.html("am"); } else { $this.html("pm"); }
    ...
    function _hasTimepicker($input) {
        if (_getTimepicker($input).length) { return true; } else { return false; }
    }
        ...
        init: function(options) {
            ...
            if (base_data.use12HourClock) {
                ...
                if (hour_range[hour_range.length - 1] < 12) {
                    ...
                    other_data.hour_data_am[other_data.hour_data_am.length - 1].max_minute = timestamp_end % 60;
                } else {
                    // the end hour is pm: save end minute in hour_data_pm
                    other_data.hour_data_pm[other_data.hour_data_pm.length - 1].max_minute = timestamp_end % 60;
                }
            }
            // default clock: 24 hour clock
                ...
                if (other_data.is_set_minmax_time) {
                    ...
                    other_data.hour_data_24[other_data.hour_data_24.length - 1].max_minute = timestamp_end % 60;
                }
            }
