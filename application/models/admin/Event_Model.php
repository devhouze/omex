...
class Event_Model extends MY_Model
{
    ...
    public function get_events($per_page,$page,$keyword,$column,$order,$count = false)
    {
        ...
        ($keyword['status']!='')?$this->db->where('te.status',$keyword['status']):$this->db->where('te.status !=',2);
    ...
    {
        $this->db->select('event_name, date_available, thumbnail_image, start_date, end_date, event_type, (case when te.status = 0 then "Active" WHEN te.status = 1 THEN "Inactive" end) as status, event_id, brand_name, event_location, event_start_time, event_end_time, about_event, event_label, event_street, event_category, show_brand, ');
        $this->db->join('tbl_admin ta','te.created_by = admin_id');
